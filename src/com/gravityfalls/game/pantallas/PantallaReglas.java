...
public class PantallaReglas implements Screen {
	...
	public void show() {
		...
		for (int i = 0; i < opciones.length; i++) {
			...
			opciones[i].setPosition(xInicial + i * (opciones[i].getAncho() + separacion * opciones.length + 35), // Posición
																													// X
																													// con
																													// separación
	...
	public void render(float delta) {
		...
		if ((entradas.isEnter() || entradas.isClick()) && opcion != -1) {
			...
			case 1:
				...
				break;
			case 2:
				game.setScreen(new PantallaControles(game)); // por ahora es una pantalla vacia, pero seria para lo del
